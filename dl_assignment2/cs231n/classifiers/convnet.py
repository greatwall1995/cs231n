...
class ConvNet(object):
  """
  [conv-relu-pool] * N - conv - relu - [affine-relu] * M - [softmax or SVM]
  """
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(L2):
      ...
      out, cache2[i] = affine_bn_relu_forward(out, W, b, gamma, beta, self.bn_params['2_' + str(i)])
    ...
    for i in xrange(L2 - 1, -1, -1):
      dout, grads['W2_' + str(i)], grads['b2_' + str(i)], grads['gamma2_' + str(i)], grads['beta2_' + str(i)] = affine_bn_relu_backward(dout, cache2[i])
...
def conv_bn_relu_pool_forward(x, w, b, conv_param, pool_param, gamma, beta, bn_param):
  ...
  c, bn_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param)
...
def conv_bn_relu_pool_backward(dout, cache):
  ...
  da, dgamma, dbeta = spatial_batchnorm_backward(dc, bn_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
  return dx, dw, db, dgamma, dbeta

def conv_bn_relu_forward(x, w, b, conv_param, pool_param, gamma, beta, bn_param):
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  c, bn_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param)
...
class ConvNet(object):
  ...
  def __init__(self, num_filters, affine_dims, filter_size, input_dim=(3, 32, 32), num_classes=10,
    ...
    drop = 1
    for i in xrange(L1):
      self.params['W1_' + str(i)] = weight_scale * np.random.randn(num_filters[i], channel, filter_size[i], filter_size[i])
      self.params['b1_' + str(i)] = np.zeros(num_filters[i])
      self.params['gamma1_' + str(i)] = np.ones(num_filters[i])
      self.params['beta1_' + str(i)] = np.zeros(num_filters[i])
      channel = num_filters[i]
      drop *= 2
    drop /= 2
    dim = (input_dim[1] / drop) * (input_dim[2] / drop) * num_filters[L1 - 1]
    ...
    self.bn_params1 = [{'mode': 'train'} for i in xrange(L1)]
    self.bn_params2 = [{'mode': 'train'} for i in xrange(L2)]
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(L1 - 1):
      W, b = self.params['W1_' + str(i)], self.params['b1_' + str(i)]
      gamma, beta = self.params['gamma1_' + str(i)], self.params['beta1_' + str(i)]
      filter_size = W.shape[2]
      conv_param = {'stride': 1, 'pad': (filter_size - 1) / 2}
      pool_param = {'pool_height': 2, 'pool_width': 2, 'stride': 2}
      out, cache1[i] = conv_bn_relu_pool_forward(out, W, b, conv_param, pool_param, gamma, beta, self.bn_params1[i])
    
    W, b = self.params['W1_' + str(L1 - 1)], self.params['b1_' + str(L1 - 1)]
    gamma, beta = self.params['gamma1_' + str(L1 - 1)], self.params['beta1_' + str(L1 - 1)]
    filter_size = W.shape[2]
    conv_param = {'stride': 1, 'pad': (filter_size - 1) / 2}
    pool_param = {'pool_height': 2, 'pool_width': 2, 'stride': 2}
    out, cache1[L1 - 1] = conv_bn_relu_forward(out, W, b, conv_param, pool_param, gamma, beta, self.bn_params1[L1 - 1])
    ...
    for i in xrange(L2):
      W, b = self.params['W2_' + str(i)], self.params['b2_' + str(i)]
      gamma, beta = self.params['gamma2_' + str(i)], self.params['beta2_' + str(i)]
      out, cache2[i] = affine_bn_forward(out, W, b, gamma, beta, self.bn_params2[i])
    ...
    loss, dout = softmax_loss(scores, y)
    for i in xrange(L1):
      loss += 0.5 * reg * (self.params['W1_' + str(i)] ** 2).sum()
    for i in xrange(L2):
      loss += 0.5 * reg * (self.params['W2_' + str(i)] ** 2).sum()
    loss += 0.5 * reg * (self.params['W3'] ** 2).sum()
    ...
    for i in xrange(L2 - 1, -1, -1):
      dout, grads['W2_' + str(i)], grads['b2_' + str(i)], grads['gamma2_' + str(i)], grads['beta2_' + str(i)] = affine_bn_backward(dout, cache2[i])
      grads['W2_' + str(i)] += reg * self.params['W2_' + str(i)]
      
    dout, grads['W1_' + str(L1 - 1)], grads['b1_' + str(L1 - 1)], grads['gamma1_' + str(L1 - 1)], grads['beta1_' + str(L1 - 1)] = conv_bn_relu_backward(dout, cache1[L1 - 1])
    grads['W1_' + str(L1 - 1)] += reg * self.params['W1_' + str(L1 - 1)]
    
    for i in xrange(L1 - 2, -1, -1):
      dout, grads['W1_' + str(i)], grads['b1_' + str(i)], grads['gamma1_' + str(i)], grads['beta1_' + str(i)] = conv_bn_relu_pool_backward(dout, cache1[i])
      grads['W1_' + str(i)] += reg * self.params['W1_' + str(i)]
